...
console.log("🚀 FINAL 7.5 SYSTEM LIVE");
...
function getSize(score,vol){
 let base=500;
 if(score>70)base=800;
 if(score<60)base=400;
 return Math.max(200,base*(0.02/vol));
}
...
// ===== MARKET TREND (FIXED RELIANCE PROXY) =====
...
setInterval(async ()=>{
 ...
 for(const SYMBOL of STOCKS){
  ...
  if(score>=60){
   if(!best||score>best.score){
    best={SYMBOL,score,crossUp,crossDown,vol};
   }
  }
 }
 ...
 if(best){
  ...
  const qty=Math.max(1,Math.floor(getSize(best.score,best.vol)/ltp));
  const price=ltp*1.001;

  if(best.crossUp && market==="BULL"){
   await kite.placeOrder("regular",{exchange:"NSE",tradingsymbol:best.SYMBOL,transaction_type:"BUY",quantity:qty,product:"MIS",order_type:"LIMIT",price});
   position={symbol:best.SYMBOL,entry:ltp,qty,side:"LONG"};
   logTrade({type:"ENTRY",...position});
   tradesToday++;
  } else if(best.crossDown && market==="BEAR"){
   await kite.placeOrder("regular",{exchange:"NSE",tradingsymbol:best.SYMBOL,transaction_type:"SELL",quantity:qty,product:"MIS",order_type:"LIMIT",price});
