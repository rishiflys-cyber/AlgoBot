
require("dotenv").config();
const express = require("express");
const path = require("path");
const { KiteConnect } = require("kiteconnect");

const app = express();
app.use(express.json());
app.use(express.static(path.join(__dirname, "public")));

const kite = new KiteConnect({ api_key: process.env.KITE_API_KEY });

let access_token=null, BOT_ACTIVE=false;
let tradesToday=0, position=null, entryPrice=0;
let pnl=0, capital=0;

const STOCKS = ["RELIANCE","HDFCBANK","ICICIBANK","INFY","TCS","SBIN","ITC","LT","AXISBANK","KOTAKBANK"];

const CONFIG = {
  MAX_TRADES:2,
  PRICE_JUMP:0.002,
  VOL_FILTER:0.0015
};

let lastPrices = {};

// LOGIN
app.get("/login",(req,res)=>res.redirect(kite.getLoginURL()));

app.get("/redirect", async (req,res)=>{
 try{
  const s=await kite.generateSession(req.query.request_token, process.env.KITE_API_SECRET);
  access_token=s.access_token;
  kite.setAccessToken(access_token);
  res.send("Login Success ✅");
 }catch{res.send("Login Failed")}
});

// CONTROL
app.get("/start",(req,res)=>{BOT_ACTIVE=true;res.send("STARTED")});
app.get("/kill",(req,res)=>{BOT_ACTIVE=false;res.send("STOPPED")});

// DASHBOARD
app.get("/dashboard", async (req,res)=>{
 try{
  if(access_token){
    const m=await kite.getMargins();
    capital=m?.equity?.net||0;
  }
 }catch{}
 res.json({capital,BOT_ACTIVE,position,tradesToday,pnl});
});

// FETCH ALL PRICES
async function getPrices(){
 try{
  const symbols = STOCKS.map(s=>`NSE:${s}`);
  const q = await kite.getLTP(symbols);
  return q;
 }catch{return null;}
}

// SCORE STOCK
function scoreStock(price, prev){
 if(!prev) return 0;
 let change=(price-prev)/prev;
 if(Math.abs(change)<CONFIG.VOL_FILTER) return 0;
 return Math.abs(change)*100;
}

// LOOP
setInterval(async ()=>{
 if(!BOT_ACTIVE || !access_token) return;
 if(tradesToday>=CONFIG.MAX_TRADES) return;

 const prices = await getPrices();
 if(!prices) return;

 let best = null;
 let bestScore = 0;

 for(let s of STOCKS){
   let p = prices[`NSE:${s}`].last_price;
   let prev = lastPrices[s];

   let sc = scoreStock(p, prev);
   if(sc > bestScore){
     bestScore = sc;
     best = {symbol:s, price:p};
   }

   lastPrices[s]=p;
 }

 if(!best || bestScore < 0.2) return;

 let type = best.price > lastPrices[best.symbol] ? "BUY" : "SELL";

 try{
  await kite.placeOrder("regular",{
    exchange:"NSE",
    tradingsymbol:best.symbol,
    transaction_type:type,
    quantity:1,
    product:"MIS",
    order_type:"MARKET"
  });

  position = best.symbol + " " + type;
  entryPrice = best.price;
  tradesToday++;

 }catch{}

},3000);

// RESET
setInterval(()=>{tradesToday=0;pnl=0},86400000);

// PORT
const PORT=process.env.PORT||3000;
app.listen(PORT,()=>console.log("MULTI STOCK BOT RUNNING",PORT));
