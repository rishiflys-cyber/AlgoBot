...
app.get("/redirect", async (req,res)=>{
  ...
  res.send("Login Success - 7.5 Version");
});
...
function atr(candles){
  ...
  for(let i=1;i<candles.length;i++){
    tr += Math.abs(candles[i].high - candles[i].low);
  }
  return tr / candles.length;
}

// Improved scoring → probability
function scoreToProb(score){
  return Math.min(0.9, 0.5 + (score-50)/100);
}
...
function getSize(score, vol){
  ...
  return Math.max(200, base * (0.02/vol)); // reduce size if volatile
}
...
setInterval(async ()=>{
  ...
  if(best){
    ...
    const prob = scoreToProb(best.score);
    if(prob < 0.6) return;

    const size = getSize(best.score, best.vol);
    const qty = Math.max(1, Math.floor(size/ltp));

    const price = ltp * 1.001; // slippage buffer (limit order)
    ...
    if(best.crossUp && market==="BULL"){
      ...
      logTrade({type:"ENTRY",...position,prob});
      tradesToday++;
